...
//
//        // ✅ FIX: Generate BCrypt hash of the raw token
...
//
//        System.out.println("before repo   :" + "hash" + hashedToken + "raw" + rawToken);
//
//
//        // ✅ FIX: Store ONLY the BCrypt hash in the 'token' field
//        RefreshToken token = RefreshToken.builder()
//                .user(user)
//                .device(device)
//                .token(hashedToken)  // ✅ Store ONLY the hash - NOT the combined format
//                .tokenId(tokenId)
//                .deviceId(deviceId)
...
//
//        System.out.println("after repo: " + savedToken.getToken());
//
...
//                .build());
//
//        // ✅ FIX: Create combined token string ONLY for response, NOT for storage
//        // The 'token' field in the entity still contains the hash
//        String combinedTokenForResponse = tokenId + "." + rawToken;
//
//        // ✅ We DON'T setToken() here - we keep the hash in the entity
//        // Instead, we set the combined token as a transient value for the response
//
//        // ✅ FIX: Use a different approach - create a DTO or use the tokenId field
//        // For now, we'll store the combined token in a separate field
//        savedToken.setToken(combinedTokenForResponse); // This will be returned but NOT saved
//
//        System.out.println("after combine     :" + savedToken.getToken());
//
...
//                .orElseThrow(() -> new InvalidTokenException("Token not found for only "));
//
//        // ✅ FIX: Get the stored hash from the database
//        String storedHash = stored.getToken();
//
//        System.out.println("repo inside stored token" + storedHash);
//
